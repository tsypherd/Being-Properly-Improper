...
#set redundancy to 10 for experiments
redundancy = 5
acc_xgb = np.zeros([num_noises,redundancy])    
acc_fixed = np.zeros([num_noises,redundancy]) 
acc_menon = np.zeros([num_noises,redundancy]) 



for noise in noises:
    for dum in range(redundancy):
        ...
        acc_xgb[noises.index(noise),dum] += accuracy
        ...
        acc_fixed[noises.index(noise),dum] += 1-test_err
        ...
        y_pred = clf.predict(X_test)[0]
        test_err = (y_pred != y_test).mean()
        acc_menon[noises.index(noise),dum] += 1-test_err
        print("PseudoMBoost Menon Accuracy:",1-test_err)

upp_low_xgb = np.zeros([num_noises,2])
upp_low_fixed = np.zeros([num_noises,2])
upp_low_menon = np.zeros([num_noises,2])
ci_xgb = np.zeros(num_noises)
ci_fixed = np.zeros(num_noises)
ci_menon = np.zeros(num_noises)
avg_acc_xgb = np.zeros(num_noises)
avg_acc_fixed = np.zeros(num_noises)
avg_acc_menon = np.zeros(num_noises)

for noise in noises:
    upp_low_xgb[noises.index(noise)] = st.t.interval(alpha=0.95, df=len(acc_xgb[noises.index(noise)])-1, loc=np.mean(acc_xgb[noises.index(noise)]), scale=st.sem(acc_xgb[noises.index(noise)])) - np.ones([1,2])*np.mean(acc_xgb[noises.index(noise)])
    upp_low_fixed[noises.index(noise)] = st.t.interval(alpha=0.95, df=len(acc_fixed[noises.index(noise)])-1, loc=np.mean(acc_fixed[noises.index(noise)]), scale=st.sem(acc_fixed[noises.index(noise)])) - np.ones([1,2])*np.mean(acc_fixed[noises.index(noise)])
    upp_low_menon[noises.index(noise)] = st.t.interval(alpha=0.95, df=len(acc_menon[noises.index(noise)])-1, loc=np.mean(acc_menon[noises.index(noise)]), scale=st.sem(acc_menon[noises.index(noise)])) - np.ones([1,2])*np.mean(acc_menon[noises.index(noise)])
    avg_acc_xgb[noises.index(noise)] = np.mean(acc_xgb[noises.index(noise)])
    avg_acc_fixed[noises.index(noise)] = np.mean(acc_fixed[noises.index(noise)])
    avg_acc_menon[noises.index(noise)] = np.mean(acc_menon[noises.index(noise)])
    
ci_xgb = upp_low_xgb[:,1]
ci_fixed = upp_low_fixed[:,1]
ci_menon = upp_low_menon[:,1]


fig, ax = plt.subplots()
ax.plot(noises,avg_acc_xgb,'k')
ax.fill_between(noises, avg_acc_xgb-ci_xgb, avg_acc_xgb+ci_xgb, color='k', alpha=.1)
ax.plot(noises,avg_acc_fixed,'r')
ax.fill_between(noises, avg_acc_fixed-ci_fixed, avg_acc_fixed+ci_fixed, color='r', alpha=.1)
ax.plot(noises,avg_acc_menon,'b')
ax.fill_between(noises, avg_acc_menon-ci_menon, avg_acc_menon+ci_menon, color='b', alpha=.1)
plt.title('Adaptive alpha Experiment')
plt.xlabel('Noise Level')
plt.legend(['XGBoost',f'Fixed PILBoost alpha = {fix_alfa}','Menon PILBoost'],loc=3)
plt.savefig(f'xd6_finshorfin_{fix_alfa}.png', dpi=1000)
plt.show()